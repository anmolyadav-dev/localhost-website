            ...
            <table className="table-auto w-full">
                <thead>
                    <tr>
                        ...
                        <th className="px-4 py-2 text-white">Project</th> {/* Project buttons column */}
                    </tr>
                </thead>
                <tbody>
                    {teams.map((team, index) => (
                        <tr key={team.teamName} className="bg-white bg-opacity-10 rounded-md mb-4 text-white text-center">
                            ...
                            <td className="px-4 py-2">{team.teamMembers.map((member) => (
                                <div key={member.name}>{member.name}</div>
                            ))}</td>
                            <td className="px-4 py-2">
                                <a href={`/hackiniiitp/projects/`}> {/* Dynamic project route TO BE ADDED ASAP*/}
                                    <button className="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded">View Project</button>
                                </a>
                            </td>
                        </tr>
                    ))}
